...
import { User, Profile } from '../API/Paths';
...
const Staffs = () => {
  ...
  const filtered = users.filter(filter =>
   filter.FirstName.toLowerCase().includes(search.toLowerCase())||
   filter.LastName.toLowerCase().includes(search.toLowerCase())||
   filter.Degree.toLowerCase().includes(search.toLowerCase()) ||
   filter.Department.toLowerCase().includes(search.toLowerCase())
  );
